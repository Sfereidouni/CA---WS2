...
public class Environment_2 : MonoBehaviour {
    ...
    void MooreLookup()
    {
        ...
        VoxelOld[] tempMONeighbors = voxel_1.GetComponent<VoxelOld>().getNeighbors3dMO();
        foreach (VoxelOld vox in tempMONeighbors)
        {
            vox.SetState(1);
		...
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < length; j++) {
				for (int k = 0; k < height; k++) {
                    ...
                    currentVoxelObj.GetComponent<VoxelOld>().SetupVoxel(i,j,k,1);
                    ...
                    if (k == 0) {						
                        ...
                        currentVoxelObj.GetComponent<VoxelOld> ().SetState (currentVoxelState);
					} else {
                        // Set the state to death
                        currentVoxelObj.GetComponent<MeshRenderer>().enabled = false;
                        currentVoxelObj.GetComponent<VoxelOld> ().SetState (0);
					}
	...
	void CalculateCA(){
		...
		for (int i = 1; i < width-1; i++) {
			for (int j = 1; j < length-1; j++) {
				...
				int currentVoxelState = currentVoxelObj.GetComponent<VoxelOld> ().GetState ();
				...
				for (int x = -1; x <= 1; x++) {
					for (int y = -1; y <= 1; y++) {
						...
						int currentNeigbourState = currentNeigbour.GetComponent<VoxelOld> ().GetState();
				...
				if (currentVoxelState == 1) {
					...
					if (aliveNeighbours < 2) {
                        currentVoxelObj.GetComponent<VoxelOld> ().SetFutureState (0);
					}
					// If there are two or three neighbours alive I am going to stay alive
					if(aliveNeighbours == 2 || aliveNeighbours == 3){
                        currentVoxelObj.GetComponent<VoxelOld> ().SetFutureState (1);
					}
	...
	void SaveCA(){
		for(int i =0; i< width; i++){
			for (int j = 0; j < length; j++) {
                ...
                savedVoxel.GetComponent<VoxelOld> ().SetState (currentVoxelState);                
                // Save the voxel age if voxel is alive
                if (currentVoxelState == 1) {
                    int currentVoxelAge = currentVoxelObj.GetComponent<VoxelOld>().GetAge();
                    savedVoxel.GetComponent<VoxelOld>().SetAge(currentVoxelAge);
                }
			}
		}
	}
